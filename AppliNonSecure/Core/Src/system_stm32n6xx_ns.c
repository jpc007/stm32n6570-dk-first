...
#if 0
  /* Appel Secure : si Hard Fault dans SECURE_SystemCoreClockUpdate / SystemCoreClockUpdate (Secure),
   * utiliser temporairement la valeur fixe ci-dessous pour confirmer et faire tourner le blink. */
  SystemCoreClock = SECURE_SystemCoreClockUpdate();
#else
  /* Contournement temporaire : valeur = horloge CPU configuree par le FSBL (PLL1 : 64/4*75 = 1200 MHz, IC1/2 = 600 MHz).
   * Sinon HAL_Delay() serait ~9x trop rapide (64 MHz au lieu de 600 MHz). Remettre l'appel Secure une fois la cause corrigee. */
