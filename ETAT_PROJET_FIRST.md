...
- **Documentation** : cette session est consignée ici ; branche `main` (dépôt `stm32n6570-dk-first`) — voir `git log` pour le dernier SHA après poussées successives. Message principal du lot fonctionnel : *Red boot screen retained; NonSecure TZ HAL_Msp fix; LED chase timing; doc* (commit `65f12d5`).
